...
from cut_cross_entropy import linear_cross_entropy # fused linear+CE: never materializes the [B*T, vocab] logits
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_logits=True):
        ...
        if targets is not None:
            ...
            # Cut Cross-Entropy fuses the lm_head matmul + softmax CE in a single
            # kernel, computing the log-sum-exp normalizer in fp32 on the fly so we
            # never allocate the [B*T, vocab] logit tensor (the old .float() copy was
            # the single biggest activation here). e and c must be bf16/fp16.
            logits = None
            loss = linear_cross_entropy(
                x.bfloat16(),
                self.lm_head.weight.bfloat16(),
                targets,
                ignore_index=-1,
            )
        else:
