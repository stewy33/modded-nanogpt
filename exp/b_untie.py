...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # untied embeddings (no weight tying)
...
class Hyperparameters:
    ...
    batch_size : int = 128 # larger batch, better MFU
    device_batch_size : int = 128 # no grad-accum
...
num_vocab = 50304
model = GPT(GPTConfig(vocab_size=num_vocab, n_layer=6, n_head=5, n_embd=640))
model = model.cuda()
...
optimizer1 = torch.optim.AdamW([*raw_model.lm_head.parameters(), *raw_model.transformer.wte.parameters()], lr=args.learning_rate, betas=(0.9, 0.95),
